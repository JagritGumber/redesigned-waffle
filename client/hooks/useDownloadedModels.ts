...
import { CivitaiModelWithRelations } from '~/backend/schema/models'; // Assuming this is the type returned by the backend
...
const getAllDownloadedModels = async (): Promise<DownloadedModelsResponse> => {
  const response = await axios.get(
    `${process.env.EXPO_PUBLIC_BACKEND_URL}/api/v1/model` // Your backend endpoint for all models
  );
  if (response.status !== 200) {
    console.error(`Failed to fetch all downloaded models:`, response.status);
    throw new Error(`Failed to fetch all downloaded models: ${response.status}`);
  }
...
const useDownloadedModels = () => {
  return useQuery<DownloadedModelsResponse, Error>({
    ...
    // Add any other desired configurations (e.g., staleTime, cacheTime)
  });
};
