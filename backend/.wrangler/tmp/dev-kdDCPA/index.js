...
__export(schema_exports, {
  ...
  authenticators: () => authenticators_default,
  civitaiFiles: () => civitaiFiles,
  civitaiImages: () => civitaiImages,
  civitaiModelVersions: () => civitaiModelVersions,
  civitaiModels: () => civitaiModels,
  groups: () => groups_default,
  sessions: () => sessions_default,
  users: () => users_default,
  verificationTokens: () => verificationTokens_default
});
    ...
    if (is(value, Table)) {
      ...
      for (const [relationName, relation] of Object.entries(relations2)) {
        if (tableName) {
          ...
          if (primaryKey4) {
            tableConfig.primaryKey.push(...primaryKey4);
          }
        } else {
...
var verificationTokens_default = verificationTokens;
...
var authenticators_default = authenticators;
...
var civitaiModels = sqliteTable("civitaiModel", {
  id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
  ...
  allowCommercialUse: text("allowCommercialUse"),
  // Could be JSON string of the array
...
var civitaiModelsRelations = relations(civitaiModels, ({ many }) => ({
  versions: many(civitaiModelVersions)
}));
var civitaiModelVersions = sqliteTable("civitaiModelVersion", {
  id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
  civitaiModelId: text("civitaiModelId").notNull().references(() => civitaiModels.id),
  ...
  // Consider using integer for timestamp if needed
  availability: text("availability"),
  nsfwLevel: integer("nsfwLevel"),
  description: text("description"),
  trainedWords: text("trainedWords"),
  // Could be JSON string of the array
  supportsGeneration: integer("supportsGeneration", { mode: "boolean" }),
  downloadUrl: text("downloadUrl"),
  // Download URL for this version (likely primary file)
  statsDownloadCount: integer("statsDownloadCount"),
  statsFavoriteCount: integer("statsFavoriteCount"),
  statsRating: real("statsRating"),
  createdAt: integer("createdAt", { mode: "timestamp_ms" }).$defaultFn(
...
var civitaiModelVersionsRelations = relations(
...
var civitaiFiles = sqliteTable("civitaiFile", {
  id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
  civitaiVersionId: text("civitaiVersionId").notNull().references(() => civitaiModelVersions.id),
...
var civitaiImagesRelations = relations(civitaiImages, ({ one }) => ({
  version: one(civitaiModelVersions, {
...
var groupRouter = new Hono2().post("/", async (c) => {
  ...
  try {
    const db = c.get("db");
    const deviantartAccounts = aliasedTable(accounts_default, "deviantart_accounts");
    const allGroupsWithRelations = await db.select({
      group: groups_default,
      patreonAccount: accounts_default,
      deviantartAccount: deviantartAccounts
    }).from(groups_default).leftJoin(accounts_default, eq(groups_default.patreonAccountId, accounts_default.id)).leftJoin(deviantartAccounts, eq(groups_default.deviantartAccountId, deviantartAccounts.id));
    if (allGroupsWithRelations && allGroupsWithRelations.length > 0) {
      ...
      const refreshPatreonToken = /* @__PURE__ */ __name(async (account) => {
        ...
        try {
          ...
          await db.update(accounts_default).set({
            access_token: tokenData.access_token,
            refresh_token: tokenData.refresh_token || account.refresh_token,
            // Use new if available, otherwise keep the old one
            expires_at: Math.floor(Date.now() / 1e3) + tokenData.expires_in
          }).where(eq(accounts_default.id, account.id));
      ...
      const refreshDeviantArtToken = /* @__PURE__ */ __name(async (account) => {
        if (!account?.refresh_token) return null;
        try {
          const tokenResponse = await fetch(
      ...
      const updatedGroupsResult = await db.select({
        group: groups_default,
        patreonAccount: accounts_default,
        deviantartAccount: deviantartAccounts
      }).from(groups_default).leftJoin(accounts_default, eq(groups_default.patreonAccountId, accounts_default.id)).leftJoin(deviantartAccounts, eq(groups_default.deviantartAccountId, deviantartAccounts.id));
  ...
  try {
    ...
    const { name } = await c.req.json();
    const updatedGroup = await db.update(groups_default).set({ name }).where(eq(groups_default.id, id)).returning();
  ...
  try {
    const db = c.get("db");
    const id = c.req.param("id");
    const deletedGroup = await db.delete(groups_default).where(eq(groups_default.id, id)).returning();
  ...
  try {
    ...
    const expiresAt = Math.floor(Date.now() / 1e3) + tokenData.expires_in;
    if (existingAccount.length === 0) {
      ...
      await db.update(accounts_default).set({
        ...
        refresh_token: tokenData.refresh_token,
        expires_at: expiresAt
      }).where(eq(accounts_default.id, patreonAccountId));
    }
    if (patreonAccountId) {
  ...
  try {
    ...
    if (existingAccount.length === 0) {
      ...
      await db.update(accounts_default).set({
        ...
        refresh_token: tokenData.refresh_token,
        expires_at: expiresAt
      }).where(eq(accounts_default.id, deviantartAccountId));
    }
...
// src/routers/v1/modelRouter.ts
var modelRouter = new Hono2().post("/", async (c) => {
  ...
  try {
    ...
    const civitaiModelData = await response.json();
    const {
      id: civitaiId,
      name,
      description,
    ...
    try {
      const [savedCivitaiModel] = await db.insert(civitaiModels).values({
        civitaiId,
        name,
        description,
        allowNoCredit,
      ...
      if (savedCivitaiModel) {
        for (const version2 of modelVersions) {
          const {
            id: civitaiVersionId,
            index,
            name: versionName,
          ...
          const [savedCivitaiModelVersion] = await db.insert(civitaiModelVersions).values({
          ...
          if (savedCivitaiModelVersion) {
            ...
            for (const image of images) {
              ...
              await db.insert(civitaiImages).values({
                civitaiVersionId: savedCivitaiModelVersion.id,
                url,
                nsfwLevel: imageNsfwLevel,
                width,
  ...
  try {
    ...
    if (allModels && allModels.length > 0) {
      ...
      return c.json({ message: "No models found" }, 200);
    }
  } catch (error) {
  ...
  try {
    ...
    const results = await db.select({
      ...
      civitaiImages,
      // Join with the images table
      eq(civitaiModelVersions.id, civitaiImages.civitaiVersionId)
      // Assuming the join is on civitaiVersionId
    ).where(eq(civitaiModels.type, "Checkpoint"));
    const modelsMap = /* @__PURE__ */ new Map();
    for (const row of results) {
      const model = row.model;
      const version2 = row.version;
      const file = row.file;
      const image = row.image;
      if (!modelsMap.has(model.id)) {
        modelsMap.set(model.id, { ...model, versions: /* @__PURE__ */ new Map() });
      }
      const modelEntry = modelsMap.get(model.id);
      if (!modelEntry.versions.has(version2.id)) {
        modelEntry.versions.set(version2.id, {
          ...version2,
          files: [],
          images: []
        });
      }
      const versionEntry = modelEntry.versions.get(version2.id);
      versionEntry.files.push(file);
      versionEntry.images.push(image);
    }
  ...
  try {
    const db = c.get("db");
    const id = c.req.param("id");
    const model = await db.select().from(civitaiModels).where(eq(civitaiModels.id, id));
...
var modelRouter_default = modelRouter;

// src/routers/v1Router.ts
var v1Router = new Hono2().get("/hi", (c) => {
  return c.text("Hello!");
}).route("/group", groupRouter_default).route("/model", modelRouter_default);
...
// .wrangler/tmp/bundle-bfdMYN/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-bfdMYN/middleware-loader.entry.ts
